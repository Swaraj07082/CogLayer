...
from qdrant_client.models import Distance, Document, PayloadSchemaType, PointStruct, VectorParams
...
def build_points(
    ...
    for user_key, memories in memory_store.items():
        ...
        for memory in memories:
            ...
            point_id = str(uuid.uuid5(uuid.UUID(qdrant_user_id), text))
            points.append(
                PointStruct(
                    id=point_id,
...
def main() -> None:
    ...
    print(f"Uploaded {len(points)} memories for {len(user_ids)} users to '{COLLECTION_NAME}'.")
